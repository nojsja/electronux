  ...
  process.on('message', ({ action, params }) => {
    switch (action) {
      case 'init-works':
        initWorks(params).then((rsp) => {
          process.send({result: rsp});
        });
        break;
      case 'upload-works':
        uploadWorks(params).then(rsp => {
          process.send({result: rsp});
        });
        break;
      case 'close':
        close(params).then(rsp => {
          process.send({result: rsp});
        });
        break;
      case 'record-set':
        ...
        break;
      case 'record-get':
        process.send({result: uploadStore.get(params)});
        break;
      case 'record-get-all':
        process.send({result: uploadStore.getAll(params)});
        break;
      case 'record-update':
